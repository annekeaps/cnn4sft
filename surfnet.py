...
class cnn4sft(object):
    ...
    def network_preparation(self):
        ...
        # Checkpoint Path
        self.retrain = False
        #self.checkpoint_path = '/home/anneke/project/surf/tensorflow/model/-150000'
        ...
        with tf.Graph().as_default():
            ...
            with tf.variable_scope(tf.get_variable_scope()):
                ...
                with tf.name_scope('project-points'):
                    # Project the points
                    xyz = tf.transpose(tf.cast(xyz_sample, tf.float64))
    ...
    def train(self):
        with tf.Graph().as_default():
            ...
            with tf.variable_scope(tf.get_variable_scope()):
                ...
                loss_deform = self.compute_loss_deformation(xyz_sample)
    ...
    def compute_loss_deformation(self, xyz_newObj):
        with tf.variable_scope('loss-deform'):
            ...
            """
            def cond(i, Ld, dist_t, dist_pred):
                return tf.less(i, size_p)

            def body(i, Ld, dist_t, dist_pred):
                p_t = points_gather_t[i:i+2, :]
                ...
                return [tf.add(i,2), Ld, dist_t, dist_pred]
            ...
            r = tf.while_loop(cond, body, [i, Ld, dist_t, dist_pred])
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.conv2d_transpose]):
            ...
            with tf.variable_scope('decoder'):
                ...
                self.conv_transpose1 = self.layer_conv_transpose(input=self.fc, kernel_size=1, num_outputs=num_outputs,
                    strides=2, padding='SAME', output_size=output_size, relu=True, bn=True)
                ...
                # Convolution
                shape = self.up_res5.shape
                num_channels = int(shape[3])
                self.conv2, self.weights_conv2 = \
                    self.layer_conv(input=self.up_res5, num_channels=num_channels, kernel_size=1,
                        num_filters=3, strides=1, padding='VALID', relu=False, bn=False)
    ...
    def layer_conv_res(self, input, num_channels, num_filters):
        out, weights = self.layer_conv(input=input, num_channels=num_channels, kernel_size=3,
            num_filters=num_filters, strides=1, padding='SAME', relu=True, bn=True)
        return out
    ...
    def downSampling_res_block(self, input, num_channels, num_filters):
        ...
        conv1, weights = self.layer_conv(input=input, num_channels=num_channels, kernel_size=1,
            num_filters=num_filters, strides=2, padding='VALID', relu=False, bn=False)
    ...
    def upSampling_res_block(self, input, num_outputs, output_size):
        ...
        conv1 = self.layer_conv_transpose(input=input, kernel_size=2, num_outputs=num_outputs, strides=2,
            padding='SAME', output_size=output_size, relu=False, bn=False)
        ...
        conv2 = self.layer_conv_transpose(input=conv1, kernel_size=2, num_outputs=num_outputs, strides=2,
            padding='SAME', output_size=output_size*2, relu=True, bn=True)
    ...
    def write_xyz(self, x, y, path, filename):
        #atom = 'C'
